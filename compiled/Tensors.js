...
class TensorOperation extends Function {
    ...
    _call(tensors, withGrad = true, ...kwargs) {
...
class Matmul extends TensorOperation {
    static tensorMul(tensors) {
        // Calculate the mean of the tensors
        let finalArr = TensorFactory.filledArray([tensors[0].length, tensors[1][0].length], 0);
    ...
    backward(gradient) {
        let grads = [Matmul.tensorMul([gradient, TensorFactory.transpose(this.t2)]), Matmul.tensorMul([this.t1, gradient])];
        return grads;
    }
